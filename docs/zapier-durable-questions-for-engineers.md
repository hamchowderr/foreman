...
>
> 1. **Which credential type / scope does durable now accept — and is it GA?** On 0.81.0 our **app-level client-credentials** token passes **all 18** durable endpoints (`runDurable`/`createWorkflow`/… → 2xx; was 403 in June). BUT we have **only** re-tested client-credentials. Our **per-user PKCE user-JWT** — the token a multi-tenant Foreman mints per end-user via the public PKCE client — is granted **`external`** scope, and in June durable's `userJwt` scheme required **`internal`** (403 under *both* client-creds and PKCE). So: **(a)** does durable now accept an **external-scope per-user PKCE** token, or only **client-credentials** (app-account) / an internal-scoped token? **(b)** Is this **GA/stable** for public-SDK clients, or an early-access state that could re-close? This decides whether Foreman runs every user's automations under one app account (client-creds) or per-user (PKCE) — a core architecture choice.
