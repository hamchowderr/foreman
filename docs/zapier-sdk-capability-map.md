...
>
> - **🔴 UPDATE 2026-07-05 (v0.81.0, foreman-e1ob) — the durable wall is GONE for CLIENT-CREDENTIALS.** A live re-probe (`scripts/durable-endpoints-probe.ts`, **client-credentials**) returns **0/18 walled** — `runDurable` / `createWorkflow` / `listWorkflows` all 2xx. Full Phase-3 pipeline + human-approval callbacks verified live under client-creds (`durable-smoke.ts` / `durable-callback-resume-probe.ts`). The 0.79.0 wall notes below are **superseded** for client-creds.
> - **⚠️ CAVEAT (foreman-e1ob) — the per-user PKCE path is NOT yet re-verified on 0.81.0.** The June 403 was proven under BOTH client-creds AND a real per-user PKCE user-JWT (`durable-pkce-probe.ts`); the 0.81.0 re-probe tested **client-creds only**. **Foreman's production durable auth is the per-user PKCE token** (`getExperimentalSdkForUser` → `zapier_identity`), which the auth model says gets `external` scope while durable's `userJwt` scheme wanted `internal`. So client-creds working does **not** prove production works — re-run `durable-pkce-probe.ts` (browser) to confirm before relying on it. Tracked in **foreman-e1ob follow-up**.
