...
>
> - **🔴 UPDATE 2026-07-05 (v0.81.0, foreman-e1ob) — the durable wall is GONE.** A live re-probe (`scripts/durable-endpoints-probe.ts`, client-creds) returns **0/18 walled** — `runDurable` / `createWorkflow` / `listWorkflows` all 2xx. Foreman's full Phase-3 durable pipeline (deploy → trigger → run → reconcile) is verified live via `scripts/durable-smoke.ts`, and human-approval callbacks resume via a plain POST (`durable-callback-resume-probe.ts`). The 0.79.0 wall notes below are **superseded** — kept for history.
