...
const TEMPLATES: Record<string, (req: string) => ExpectedBehavior> = {
  "one-shot-action": (req) => ({
    ...
    forbidden_tools: [
      "run-action[app=zapier-tables]",
      "create-table",
    ],
  }),

  "save-as-workflow": (req) => ({
  ...
  "channel-triggered-workflow": (req) => ({
...
async function main() {
  ...
  console.log(`Mode: ${dryRun ? "DRY-RUN" : "LIVE"}${representative ? " | representative-only" : ""}${Number.isFinite(limit) ? ` | limit=${limit}` : ""}\n`);
  ...
  for (const item of candidates) {
    ...
    console.log(`  REQUEST: ${input.request.slice(0, 100)}${input.request.length > 100 ? "…" : ""}`);
    console.log(`  TOOLS:   ${label.expected_tools.length === 0 ? "(none)" : label.expected_tools.join(" → ")}`);
    console.log(`  FORBID:  ${label.forbidden_tools.length === 0 ? "(none)" : label.forbidden_tools.join(", ")}`);
  ...
  console.log("=== DONE ===");
  console.log(`${dryRun ? "Would have updated" : "Updated"}: ${dryRun ? candidates.length : updated} item(s)`);
  if (!dryRun) {
