...
import { Check, ChevronRight, GitBranch, Play, Sparkles } from "@/components/icons/hi";
...
const SCENARIOS: Scenario[] = [
  {
    ...
    chat: [
      {
        role: "user",
        text: "Every Monday, email the team standup link and create a Linear issue for blockers.",
      },
  ...
  },
  {
    ...
    chat: [
      {
        role: "user",
        text: "When a customer signs up in Stripe, add them to Notion CRM and send the welcome email.",
      },
  ...
  },
  {
    ...
    chat: [
      {
        role: "user",
        text: "End of sprint: close done Linear tickets, post a summary to #general, schedule the retro.",
      },
  ...
  },
  {
    ...
    chat: [
      {
        role: "user",
        text: "Every Friday afternoon, post this week's wins to Slack and export the metrics to Google Sheets.",
      },
  ...
  useEffect(() => {
    ...
    const advance = () => {
      ...
      if (step.phase === "running") {
        ...
        runTimers.push(
          setTimeout(() => setRunStep(scenario.steps.length), 300 + scenario.steps.length * 800),
        );
      }
              ...
              {phase === "chat" || phase === "highlight" ? (
                ...
                >
                  ...
                  {scenario.steps.map((step, i) => {
                    ...
                    const complete = (phase === "running" && runStep > i) || phase === "done";
                    return (
                      <motion.div
                        key={`${scenario.id}-${i}`}
