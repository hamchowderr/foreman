...
import { Check, ChevronDown, ChevronRight, Clock, Search, ShieldAlert, X, Zap } from "lucide-react";
...
const SCENARIOS: Scenario[] = [
  {
    ...
    script: [
      ...
      {
        item: { kind: "result", text: 'Row added to "Q2 Sales Tracker" — 39 total records.' },
        delay: 0,
      },
    ],
  },
  {
    ...
    script: [
      ...
      { item: { kind: "approved" }, delay: 600 },
      {
        item: { kind: "result", text: "Workflow complete — digest sent to 12 subscribers." },
        delay: 0,
      },
    ],
  },
    ...
    <div className="rounded-xl border border-white/[0.07] bg-black/50 backdrop-blur-sm overflow-hidden font-mono text-sm flex flex-col">
      ...
      <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-white/[0.05] bg-white/[0.015] shrink-0">
        ...
        <div className="ml-auto flex items-center gap-1">
          {SCENARIOS.map((_s, i) => (
            <div
              // biome-ignore lint/suspicious/noArrayIndexKey: SCENARIOS is a static const array; index is a stable identity
              key={i}
      ...
      <div
        ref={scrollRef}
        className="flex-1 overflow-y-auto p-4 space-y-2.5 min-h-[220px] max-h-[260px] [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
      >
        {items.map((item, i) => (
          // biome-ignore lint/suspicious/noArrayIndexKey: composite key with scenarioIdx ensures remount on scenario change
    ...
    return (
      <div className="flex items-center gap-2 animate-fade-in">
        ...
        <div className="flex gap-1 items-center">
          <span
            className="h-1.5 w-1.5 rounded-full bg-white/20 animate-bounce"
      ...
      <div className="animate-fade-in">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            {hasResult && <span className="text-emerald-400/50 text-[10px]">✓</span>}
            {expanded ? (
    ...
    return (
      <div className="animate-fade-in rounded-xl border border-[#ff4a00]/30 bg-[#ff4a00]/[0.04] overflow-hidden">
        <div className="flex items-center gap-2 px-3 py-2 border-b border-[#ff4a00]/15 bg-[#ff4a00]/[0.06]">
          ...
          <span className="text-[#ff4a00] text-[11px] font-semibold tracking-wide uppercase">
            Approval Required
          </span>
        </div>
