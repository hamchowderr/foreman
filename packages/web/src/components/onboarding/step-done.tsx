...
const FEATURES = [
  ...
  },
  {
    ...
    description: 'No flows to build. Describe what you need in plain English and Foreman figures out the rest.',
    color: '#2B2358',
    bg: '#F0EEFF',
    border: '#C4BAF0',
  },
        ...
        {FEATURES.map(({ icon: Icon, label, description, color, bg, border }) => (
          ...
          >
            ...
            <div className="space-y-0.5">
              <p className="text-sm font-semibold leading-snug" style={{ color: '#201515' }}>{label}</p>
              <p className="text-xs leading-relaxed" style={{ color: '#6B5050' }}>{description}</p>
            </div>
          </motion.div>
        ))}
      </motion.div>
