...
export function OnboardingFlow({ initialStep, initialUses, zapierJustConnected }: Props) {
  ...
  const _goTo = (s: number) => setStep(s);
    ...
    <div className="min-h-svh flex flex-col" style={{ backgroundColor: "#FFFDF9" }}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {Array.from({ length: STEPS }).map((_, i) => (
            <div
              // biome-ignore lint/suspicious/noArrayIndexKey: STEPS is a constant; index is the stable identity
              key={i}
      ...
      <div className="flex-1 flex items-start justify-center px-6 py-12">
        <div className="w-full max-w-2xl">
          {step === 0 && <StepWelcome selected={uses} onSelect={setUses} onNext={goNext} />}
          {step === 1 && (
            <StepConnect uses={uses} zapierJustConnected={zapierJustConnected} onNext={goNext} />
          )}
          {step === 2 && <StepTry uses={uses} onNext={goNext} />}
          {step === 3 && <StepDone />}
        </div>
      </div>
    </div>
